...
RUN apt-get update && apt-get install -y \
    ...
    libgtk-3-0 libnotify4 net-tools iproute2 iptables \
...
# Create startup script with PROPER VPN routing
RUN echo '#!/bin/bash\n\necho "=== VPN Setup === "\n\n# Function to check current IP\ncheck_ip() {\n    echo "🌐 Current IP: $(curl -s --max-time 10 https://api.ipify.org || echo "Unknown")"\n}\n\necho "Initial IP:"\ncheck_ip\n\n# Get working VPN config\necho "Finding VPN server..."\npython3 /app/find_vpn.py\n\nif [ -f "/tmp/vpn.ovpn" ]; then\n    echo "Starting OpenVPN with proper routing..."\n    \n    # Modify the config to ensure all traffic goes through VPN\n    echo "route-nopull" >> /tmp/vpn.ovpn\n    echo "redirect-gateway def1" >> /tmp/vpn.ovpn\n    echo "script-security 2" >> /tmp/vpn.ovpn\n    echo "up /app/vpn-up.sh" >> /tmp/vpn.ovpn\n    \n    # Start OpenVPN\n    openvpn --config /tmp/vpn.ovpn --daemon\n    \n    # Wait for VPN to establish\n    echo "Waiting for VPN connection..."\n    sleep 25\n    \n    # Check if VPN changed IP\n    echo "Checking VPN status..."\n    check_ip\n    \n    # Add kill switch - block non-VPN traffic\n    iptables -A OUTPUT ! -o tun0 -m state --state NEW -j DROP 2>/dev/null || true\n    \nelse\n    echo "❌ No VPN config available"\nfi\n\necho "=== Starting FeelingSurf === "\n\n# Start services\nservice dbus start\nXvfb :99 -screen 0 1024x768x24 2>/dev/null &\nexport DISPLAY=:99\nsleep 5\n\n# Start FeelingSurf\ncd /app\nwhile true; do\n    echo "Starting FeelingSurf Viewer..."\n    ./FeelingSurfViewer --access-token d6e659ba6b59c9866fba8ff01bc56e04 --no-sandbox --disable-dev-shm-usage\n    echo "Viewer exited. Restarting in 30 seconds..."\n    sleep 30\ndone' > /app/start.sh && chmod +x /app/start.sh

# Create VPN up script for routing
RUN echo '#!/bin/bash\n\necho "VPN interface $1 is up"\n\n# Force all traffic through VPN\nip route add default via $route_vpn_gateway dev $1 table 100 2>/dev/null || true\nip rule add from all lookup 100 2>/dev/null || true\n\n# Remove default route that might bypass VPN\nip route del default via 172.17.0.1 2>/dev/null || true\nip route del default via 172.18.0.1 2>/dev/null || true\n\necho "VPN routing configured"' > /app/vpn-up.sh && chmod +x /app/vpn-up.sh
